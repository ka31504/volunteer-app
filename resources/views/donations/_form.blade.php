...
<div class="p-form-grid" x-data="donationForm({
    ...
    <div class="p-form-section">
        ...
        <div class="p-field">
            ...
            <input id="donor_name" type="text" name="donor_name" value="{{ old('donor_name', $d?->donor_name) }}"
                class="p-input" required>
            @error('donor_name')<div class="p-error">{{ $message }}</div>@enderror
        </div>

        <div class="p-field">
            <label class="p-label" for="sponsor_id">Nhà tài trợ (nếu có)</label>
            <select id="sponsor_id" name="sponsor_id" class="p-input">
                <option value="">— Không liên kết —</option>
                @foreach($sponsors as $sponsor)
                <option value="{{ $sponsor->id }}" {{ old('sponsor_id', $d?->sponsor_id) == $sponsor->id ? 'selected' :
                    '' }}>
                    {{ $sponsor->name }} ({{ $sponsor->type_label }})
                </option>
                @endforeach
            </select>
            @error('sponsor_id')<div class="p-error">{{ $message }}</div>@enderror
        </div>

        <div class="p-field">
            <label class="p-label" for="donor_phone">Số điện thoại</label>
            <input id="donor_phone" type="tel" name="donor_phone" value="{{ old('donor_phone', $d?->donor_phone) }}"
                class="p-input">
        </div>
        ...
        <div class="p-field">
            ...
            <div class="flex gap-6 pt-1">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="type" value="money" x-model="type" class="accent-[var(--primary)]"
                        required>
        ...
        <div x-show="type === 'money'" x-transition>
            <div class="p-field">
                ...
                <div class="p-input-wrap">
                    <input id="amount" type="number" name="amount" value="{{ old('amount', $d?->amount) }}"
                        class="p-input" min="1000" step="1000" :required="type === 'money'"
                        :disabled="type !== 'money'">
            ...
            <div class="p-field">
                ...
                <select id="payment_method" name="payment_method" class="p-input">
                    <option value="cash" {{ old('payment_method', $d?->payment_method ?? 'cash') === 'cash' ? 'selected'
                        : '' }}>Tiền mặt</option>
                    <option value="transfer" {{ old('payment_method', $d?->payment_method) === 'transfer' ? 'selected' :
                        '' }}>Chuyển khoản</option>
                    <option value="other" {{ old('payment_method', $d?->payment_method) === 'other' ? 'selected' : ''
                        }}>Khác</option>
                </select>
            </div>
        </div>
        ...
        <div x-show="type === 'goods'" x-transition>
            ...
            <div class="p-field">
                ...
                <input id="goods_description" type="text" name="goods_description"
                    value="{{ old('goods_description', $d?->goods_description) }}" class="p-input"
                    :required="type === 'goods'">
            ...
            <div class="p-field">
                ...
                <input id="goods_quantity" type="number" name="goods_quantity"
                    value="{{ old('goods_quantity', $d?->goods_quantity) }}" class="p-input" min="1"
                    :required="type === 'goods'">
